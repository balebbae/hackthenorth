---
source: https://www.nianticspatial.com/docs/nsdk/how-to/ar/depth/convert_point_world_position/
...
You will need a Unity project with Niantic Spatial AR enabled. For more information, see [Set up the Niantic SDK for Unity](https://www.nianticspatial.com/docs/nsdk/setup/#set-up-the-niantic-sdk-for-unity).
...
If this is your first time using depth, [Accessing and Displaying Depth Information](https://www.nianticspatial.com/docs/nsdk/how-to/ar/depth/display_depth/) provides a simpler use case for depth and is easier to start with.
...
6.  Calculate the display matrix: Because depth images are oriented towards the sensor when surfaced from the Machine Learning model, they need to be sampled with respect to the current screen orientation. The display transform provides a mapping to convert from screen space to the image coordinate system. We use `XRCpuImage` instead of a GPU Texture so that the `Sample(Vector2 uv, Matrix4x4 transform)` method can be used on the CPU.
    ...
    5.  Call `CalculateDisplayMatrix` to calculate a matrix that transforms the screen coordinates to image coordinates.
...
7.  Set up code to Handle Touch Inputs:
    ...
    2.  In editor, we'll use `Mouse.current.leftButton.wasPressedThisFrame` to detect mouse clicks.
    3.  On device, we'll use `Touchscreen.current.primaryTouch.press.wasPressedThisFrame` to detect taps.
...
13. Try running the scene in-editor using [Playback](https://www.nianticspatial.com/docs/nsdk/how-to/playback/setting_up_playback/) or open **Build Settings**, then click **Build and Run** to build to device.
...
- [Depth Feature page](https://www.nianticspatial.com/docs/nsdk/features/depth/)
- [Occlusion Feature page](https://www.nianticspatial.com/docs/nsdk/features/occlusion/)
